...
//            pto.setPosition(newPosition);
        ...
        if(gamepad1.dpadRightWasPressed()) {
            double newPosition =  gatePusher.getPosition()+.01;
            newPosition = newPosition > 1 ? 1 : newPosition;
////            gate.setPosition(.537);//closed
            gatePusher.setPosition(newPosition);
        }
...
//        telemetry.addData("gate pos", gate.getPosition());
        telemetry.addData("currret gatePusher pos", gatePusher.getPosition());
        telemetry.update();
    }
}
