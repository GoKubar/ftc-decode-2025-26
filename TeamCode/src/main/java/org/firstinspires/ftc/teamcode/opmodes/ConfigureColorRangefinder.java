...
/**
 * Run this opmode ONCE (while sensor is on I2C, configured as "Rev Color Sensor V3" named "Color")
 * to write thresholds persistently to the sensor. After it runs, move the sensor to a digital port.
 *
 * Sensor must be in I2C mode to configure. If it's already in digital mode, follow the
 * reset sequence in the Brushland docs to get it back to I2C first.
 */
@TeleOp(name = "Configure Color Rangefinder", group = "Test")
public class ConfigureColorRangefinder extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        telemetry.addLine("Ready — press START to write config to sensor");
        telemetry.addLine("LED will blink twice on success");
        telemetry.update();

        waitForStart();

        // --- Tune these thresholds for your game pieces ---
        // Hue bounds: scale 0-360° to 0-255 (0° = red/top, clockwise)
        // pin0 = purple, pin1 = green (example from Brushland docs)

        crf.setPin0Digital(ColorRangefinder.DigitalMode.HSV, 160 / 360.0 * 255, 190 / 360.0 * 255); // purple
        crf.setPin0DigitalMaxDistance(ColorRangefinder.DigitalMode.HSV, 10); // within 10mm

        crf.setPin1Digital(ColorRangefinder.DigitalMode.HSV, 110 / 360.0 * 255, 140 / 360.0 * 255); // green
        crf.setPin1DigitalMaxDistance(ColorRangefinder.DigitalMode.HSV, 10); // within 10mm

        telemetry.addLine("Config written! Move sensor to digital port.");
        telemetry.update();
 ...
 * Source: https://docs.brushlandlabs.com/sensors/color-rangefinder/configuration
 */
class ColorRangefinder {
    ...
    private void setDigital(PinNum pinNum, DigitalMode digitalMode, double lowerBound, double higherBound) {
        int lo, hi;
        if (lowerBound == higherBound) {
            lo = (int) lowerBound;
            hi = (int) higherBound;
        } else if (digitalMode.value <= DigitalMode.HSV.value) {
